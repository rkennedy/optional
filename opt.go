// Package optional provides an “option type,” which can either be empty or hold a value. In that respect, it's very
// similar to an ordinary pointer type, except it has methods that make its possible emptiness more explicit.
//
// The interface of the [Value] type is modeled on Java's [java.util.Optional] type and C++'s [std::optional].
//
// Also included are some Gomega matchers for use in tests.
//
...
// Value is a type that may or may not hold a value. Its interface is modeled on Java's [java.util.Optional] type and
// C++'s [std::optional].
//
...
// OrElseGet returns the stored value, if there is one. If the Value is empty, then OrElseGet calls the given function
// and returns the result. Use this instead of OrElse when calculation of the fallback value is relatively expensive. It
...
// TransformWithError applies the given function to the optional value if the input value is non-empty, and returns a
// new optional of the corresponding return type holding the returned value. Returns an empty value if the input is
// empty. If the transform function returns an error, then an empty value and that error are returned.
